...
- 위와 조건, 재귀함수 설계까지 모두 같지만, 포인트는 '오름차순'이라는 것
  - 오름차순을 걸러내기 위해서는 `(해당 값 + 1)`부터 반복문을 걸러내면 된다.
...
```py
start_num = _list[-1] if len(_list) else 0 # 배열의 길이가 0일 수도 있으니..
...
def permutation():
    ...
    for i in range(_list[-1] + 1, N+1):
        # 단, 배열에 아무것도 없을 수 있으므로 초기값은 잘 설정해야한다.
        _list.append(i)
        permutation()
        _list.pop()
```
